...
class UnivParser(SGMLParser):
    ...
    def handle_data(self, data):
        if self.insideUnivLink:
            self.currentUniv['university_name'] = unicode( data.strip().replace('"', '\\"'), 'latin-1')
            print self.currentUniv['university_name']
...
def buildSchools():
    ...
    cursor.execute("CREATE TABLE %s(`id` INT(11) NOT NULL
            AUTO_INCREMENT,`country_id` INT(5) NOT NULL,`university_name` VARCHAR(150) NOT NULL, `url` VARCHAR(150) NOT NULL, PRIMARY KEY (`id`), KEY `country_id` (`country_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8" % (TBL_SCHOOL))
    ...
    for country in countries:
        ...
        for school in schools:
            cursor.execute('''INSERT INTO %s VALUES(NULL, %d, "%s", "%s")''' %
                    (TBL_SCHOOL, country[0], school['university_name'], school['link']))
...
def insertSchools(url, cc):
    ...
    for school in schools:
        cursor.execute('''INSERT INTO %s VALUES(NULL, %d, "%s", "%s")''' %
                (TBL_SCHOOL, country[0][0], school['university_name'], school['link']))
    print "\nEnd building schools."
